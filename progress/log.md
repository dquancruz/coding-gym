...
## 2026-07-16 — [Node] 0002 reservas-stock — verdict (🟨, fixed same day — see **Fix applied** below)
**Score:** Correctness 4 · Readability 4 · Idiomatic 4 · Tests 3 · Errors 2 · Design 4 · Performance 4 · Explanation 4 (original submission; see fix note for updated scores)
**Learned:** Second Node exercise and the layering is exemplary, not just repeated from 0001: `routes/` → `controller/` → `service/` → `repository/` is strictly one-directional (verified by reading every file — repositories never import Express types, services never import `req`/`res`), and the composition root (`createApp()`) correctly builds a fresh dependency graph per call so tests get isolated state (confirmed: `beforeEach(() => app = createApp())` in both test files, 22/22 passing via independent `vitest run`). `stockDisponible` is genuinely never stored, only derived in `toProductoView` — this structurally prevents the desync bug the README frames the whole design around. The `AppError` abstract-class hierarchy (`NotFoundError`/`ConflictError`, each owning its own `statusCode`/`code`) is a clean idiom, and the 409-vs-400-vs-422 write-up in the README is genuine, specific reasoning (not boilerplate) — same Mid→Senior-flavored tradeoff communication seen in the TS track. Idempotent-cancel is correctly implemented with an early `return` before the stock-release call, and independently verified via a dedicated test plus my own read of `ReservasService.cancelar`. `tsc --noEmit` compiles clean under a notably strict config (`noUncheckedIndexedAccess`, `exactOptionalPropertyTypes`, `noUnusedLocals` all on, not just bare `strict`). Boundary and cross-entity tests are present and meaningful: reserving exactly the disponible amount succeeds, one more fails; cancelling product A's reservation is verified to leave product B's identical-quantity reservation untouched — the "two related identities, validate the relationship" lesson from the TS track transferred here correctly.
**To improve:** The central error handler (`src/middleware/errorHandler.ts`) always returns `500` for anything that isn't an `AppError` — this is the **exact bug already flagged and fixed in 0001** (`log.md`, 2026-07-09), now reintroduced in a brand-new file. Verified directly: `POST /productos` with a malformed JSON body (`Content-Type: application/json`, unparseable payload) returns `500 {"error":{"code":"INTERNAL_ERROR",...}}`, even though the underlying error `body-parser` throws already carries `statusCode: 400, status: 400, expose: true` (confirmed by logging the raw caught error) — the handler never checks for it. None of the 22 tests exercise this path at all, so nothing caught it before I probed manually — same "did the failure path actually get run" gap noted on 0001 and 0005. This one matters more than usual: it's not new territory, it's a previously-corrected mistake recurring in the one skill this exercise was explicitly built to practice (`centralized error handling`, tracker row, still 0/3). Secondary, non-blocking: `ReservasRepository.buscarPorId`/`ReservasService.obtener` return the *actual internal object reference*, not a defensive copy — unlike `ProductosService.obtener`, which correctly builds a fresh object via `toProductoView`. Verified with a standalone script: calling `reservasService.obtener(id)` and mutating the returned object's `estado`/`cantidad` directly corrupts the repository's internal state with zero validation, and leaves `stockReservado` on the product silently unadjusted (i.e., the reserva now claims `cancelada`/`cantidad: 999` while the product still thinks 3 units are reserved — the exact kind of desync the whole exercise is designed to prevent structurally). Not reachable through the current HTTP surface (Express serializes to JSON before handing anything to the client), so it's a latent encapsulation gap, not a live bug — but it's the same bug species as 0005's `Date`-mutation leak, and it's inconsistent within this same file: `ProductosService` already defends against it, `ReservasService` doesn't.
**Suggested next refactor:**
```ts
// Before (errorHandler.ts) — every non-AppError becomes 500, even ones that already know their status
export function errorHandler(err: unknown, _req: Request, res: Response, _next: NextFunction): void {
  ...
  res.status(500).json({ error: { code: 'INTERNAL_ERROR', message: 'Ocurrió un error interno inesperado' } });
}

// After — trust a statusCode the error already carries (body-parser, etc.); default to 500 only for genuinely unknown errors
...
export function errorHandler(err: unknown, _req: Request, res: Response, _next: NextFunction): void {
  ...
  const statusCode = tieneStatusCode(err) && err.statusCode < 500 ? err.statusCode : 500;
  if (statusCode >= 500) console.error('[error-inesperado]', err);
  res.status(statusCode).json({
    error: {
      code: statusCode < 500 ? 'BAD_REQUEST' : 'INTERNAL_ERROR',
      message: statusCode < 500 ? 'La petición no pudo procesarse' : 'Ocurrió un error interno inesperado',
    },
  });
}
```
Add the regression test that would've caught it (same shape as the one suggested — and skipped — on 0001): `.set('Content-Type', 'application/json').send('{nombre: bad json')` → assert `400`, not `500`. Lower priority: make `ReservasRepository.buscarPorId` and `ReservasService.obtener`/`listarActivasPorProducto` return `{ ...reserva }` copies, matching the defensive pattern `toProductoView` already establishes for products.
**Fix applied (2026-07-16, by Claude Code at the user's explicit request — not an independent learner resubmission):** `errorHandler.ts` now checks for an upstream `statusCode` in the 4xx range (`tieneStatusCodeDeCliente`) before defaulting to 500 — a malformed-JSON body now correctly returns `400 {"code":"BAD_REQUEST",...}`, verified directly against a live `createApp()` instance. `ReservasService.crear`/`obtener`/`cancelar`/`listarActivasPorProducto` now return `{ ...reserva }` copies instead of live internal references, matching `toProductoView`'s pattern. Added `tests/errorHandler.test.ts` (malformed-JSON → 400, unknown route → 404 JSON) and `tests/reservas.service.test.ts` (mutating a returned `Reserva` no longer corrupts repository state or `stockReservado`) — full suite now 25/25 (`vitest run`), `tsc --noEmit` clean. Re-scored: **Errors 4** (statusCode-aware, tested), **Tests 4** (both previously-missing failure paths now pinned down) — all dimensions ≥3, average 4.0, verdict **🟩**.
**Important bookkeeping note:** this fix was written by Claude Code, not re-attempted by the learner from the review feedback alone — the code is correct and the exercise is complete/mergeable, but it does **not** demonstrate the learner independently applying the "trust an upstream statusCode" pattern. Do not count this toward the 3-consecutive-🟩 promotion streak for `centralized error handling`; that skill still needs a genuine learner attempt. `basic REST + layered structure` legitimately earned this evidence point on the original submission regardless (architecture/tests/business-logic were the learner's own work and were clean).
**Next skill to rotate in:** `basic REST + layered structure` is now 2/3 toward `level up` (0001, 0001-fix, 0002 — all learner-authored and 🟩-quality on this skill's dimensions); one more clean 🟩 on a new exercise qualifies it. `centralized error handling` stays at 0/3 pending a real learner attempt — good next exercise to assign, ideally one that forces reasoning about which errors already carry a status vs. which are genuinely unknown. TypeScript's three skills sitting at 2/3 (basic types, generics, discriminated unions) remain the closest to `level up`. Zod is 1/3. SQL, Python, Docker, Java, .NET, and Rails remain at 0/3.
